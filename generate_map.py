...
for group_id, (coords, groupe) in enumerate(acteurs_par_gps.items()):
    ...
    for pos_in_group, (idx, actor) in enumerate(groupe):
        ...
        bloc_contact_html = f'<a href="mailto:{email}" style="color:#2D3277; font-weight:bold; text-decoration:none;">{email}</a>' if email and "@" in email else (email or '<span style="color:#999; font-style:italic;">Non renseigné</span>')
